...
const dealerResult = (cardArray, boardArray, playerCount, playerCardCount) => {
  ...
  */
  ...
  let playerWinsBoard1 = []
  let playerWinsBoard2 = []
  let playerScoop = []



  for(i = 0; i < playerCount; i++) {
    playerWinsBoard1[i] = 0
    playerWinsBoard2[i] = 0
    playerScoop[i] = 0
  }

  for(i = 0; i < playerCount; i++) {
    if(isHighScore1[i] == 1) {
      playerWinsBoard1[i] += 1/countHighScores1
    }
    if(isHighScore2[i] == 1) {
      playerWinsBoard2[i] += 1/countHighScores2
    }
  ...
  return {
    gameHasHigh: true,
    gameHasLow: true, 
    playerWinsHigh: playerWinsBoard1,
    playerWinsLow: playerWinsBoard2,
